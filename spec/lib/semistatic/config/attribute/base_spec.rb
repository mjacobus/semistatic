...
describe Semistatic::Config::Attribute::Base, '#type' do
  subject { described_class.new(:name) }
  its(:type) { should eq(:base) }
end

describe Semistatic::Config::Attribute::Base, '#name' do
  subject { described_class.new(:name) }
  its(:name) { should eq('name') }
end

describe Semistatic::Config::Attribute::Base, '#description' do
  subject { described_class.new(:name, { description: 'a description'}) }
  its(:description) { should eq('a description') }
end

describe Semistatic::Config::Attribute::Base, '#label' do
