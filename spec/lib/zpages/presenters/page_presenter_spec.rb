...
describe Zpages::Presenters::PagePresenter do
  let(:helpers) { ApplicationController.helpers }
  let(:string_part)  { FactoryGirl.build_stubbed(:zpages_part, options: { type: :string }, name: :string) }
  let(:html_part)    { FactoryGirl.build_stubbed(:zpages_part, options: { type: :html   }, name: :html) }
  let(:image_part)   { FactoryGirl.build_stubbed(:zpages_part, options: { type: :image  }, name: :image) }

  let(:page) do
    page = FactoryGirl.build_stubbed(:zpages_page, title: 'page title', parts: [string_part, html_part, image_part])
